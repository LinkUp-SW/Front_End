...
const SavedJobsDashboard: React.FC = () => {
  ...
  const loadAppliedJobs = async () => {
    ...
    try {
      ...
      const jobs = jobsData.map(jobData => convertAppliedJobDataToJob(jobData));
      setAppliedJobs(jobs);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      window.removeEventListener('appliedJobsUpdated', handleAppliedJobsUpdated);
    };
  }, [activeTab]);
      ...
      return (
        <div className="border-t pt-6 pb-4 text-center">
          <div className="flex flex-col items-center justify-center py-8">
            ...
            />
            <p className="text-gray-500 dark:text-gray-400 text-2xl">No saved jobs.</p>
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
    ...
    if (activeTab === TabState.APPLIED && appliedJobs.length === 0) {
      return (
        <div className="border-t pt-6 pb-4 text-center">
          <div className="flex flex-col items-center justify-center py-8">
            <img 
              src={isDarkMode ? '/src/assets/no-applied-jobs-dark.svg' : '/src/assets/no-applied-jobs-light.svg'} 
              alt="No applied jobs" 
              className="w-48 h-48 mb-4"
            />
            <p className="text-gray-500 dark:text-gray-400 text-2xl">No applied jobs.</p>
            <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">
                ...
                <div className="flex">
                  ...
                  <div className="ml-3">
                    ...
                    <div className="flex items-center mt-1">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${getStatusBadgeColor(job.application_status)}`}>
