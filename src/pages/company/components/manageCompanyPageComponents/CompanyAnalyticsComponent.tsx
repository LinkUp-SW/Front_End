...
const AnalyticsComponent: React.FC<AnalyticsComponentProps> = ({ companyId }) => {
    ...
    return (
      <div className="flex justify-center items-center h-64">
        <div className="flex flex-col items-center">
          ...
          <div className="mt-4 text-gray-600 dark:text-gray-300 font-medium">Loading analytics...</div>
        </div>
      </div>
    );
  }
  ...
  if (!analytics) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-gray-700 dark:text-gray-300">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-2">Try again later or check your company settings</p>
      </div>
    );
  }
  ...
  const pendingPercentage = ((analytics.applicationStatusDistribution.Pending || 0) / totalApplications) * 100;
  const acceptedPercentage = ((analytics.applicationStatusDistribution.Accepted || 0) / totalApplications) * 100;
  const rejectedPercentage = ((analytics.applicationStatusDistribution.Rejected || 0) / totalApplications) * 100;
  const viewedPercentage = ((analytics.applicationStatusDistribution.Viewed || 0) / totalApplications) * 100;

  return (
    <div className={`space-y-8 transition-opacity duration-500 ${showAnimation ? 'opacity-100' : 'opacity-0'}`}>
      {/* Dashboard Title */}
        ...
        <div className="bg-gradient-to-br from-green-50 to-green-100 dark:from-gray-700 dark:to-gray-800 p-6 rounded-xl shadow-sm border border-green-200 dark:border-gray-600 transform transition-all hover:shadow-md hover:-translate-y-1">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-3xl font-bold text-green-600 dark:text-green-400">{analytics.openJobs}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-600 transform transition-all hover:shadow-md hover:-translate-y-1">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-3xl font-bold text-gray-600 dark:text-gray-300">{analytics.closedJobs}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-gray-700 dark:to-gray-800 p-6 rounded-xl shadow-sm border border-purple-200 dark:border-gray-600 transform transition-all hover:shadow-md hover:-translate-y-1">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-3xl font-bold text-purple-600 dark:text-purple-400">{analytics.totalApplications}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="flex items-center mb-6">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white">Application Status Distribution</h3>
        </div>
        ...
        <div className="grid grid-cols-1 gap-6">
          <div className="flex flex-col">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                {analytics.applicationStatusDistribution.Pending || 0} ({Math.round(pendingPercentage)}%)
              </span>
            </div>
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                {analytics.applicationStatusDistribution.Accepted || 0} ({Math.round(acceptedPercentage)}%)
              </span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div 
          ...
          <div className="flex flex-col">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                {analytics.applicationStatusDistribution.Rejected || 0} ({Math.round(rejectedPercentage)}%)
              </span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
              <div 
          ...
          {analytics.applicationStatusDistribution.Viewed && (
            <div className="flex flex-col">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">
                  {analytics.applicationStatusDistribution.Viewed} ({Math.round(viewedPercentage)}%)
                </span>
              </div>
              <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2.5">
                <div 
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="flex items-center mb-6">
          ...
          <h3 className="text-lg font-medium text-gray-900 dark:text-white">Top Jobs by Applications</h3>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead>
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider bg-gray-50 dark:bg-gray-700 rounded-tl-lg">
                  Job Title
                </th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider bg-gray-50 dark:bg-gray-700 rounded-tr-lg">
                  Applications
                </th>
              </tr>
            </thead>
                      ...
                      <div className="ml-2 w-24 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full ${
                            index === 0 ? 'bg-blue-500' : 'bg-gray-500 dark:bg-gray-400'
                          }`}
                          style={{ 
                            width: `${(job.applications_count / analytics.topJobs[0].applications_count) * 100}%` 
                          }}
                        ></div>
                      </div>
                    </div>
