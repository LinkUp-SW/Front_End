...
const ContentModeration = () => {
  ...
  return (
    <div className="bg-gradient-to-b from-white via-blue-50 to-blue-100 dark:from-gray-800 dark:via-gray-900 dark:to-gray-950 rounded-2xl shadow-xl p-6 transform transition duration-300 mb-12">
      <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">Content Moderation</h2>
      <p className="text-gray-600 dark:text-gray-300 mb-6">Review and manage reported content across the platform.</p>
      ...
      <div className="flex border-b border-gray-200 dark:border-gray-700 mb-6">
        <button
          className={`px-4 py-2 font-medium ${activeTab === "Pending" ? "text-blue-500 border-b-2 border-blue-500" : "text-gray-500 dark:text-gray-400"}`}
          onClick={() => setActiveTab("Pending")}
        >
          Pending
        </button>
        <button
          className={`px-4 py-2 font-medium ${activeTab === "Resolved" ? "text-blue-500 border-b-2 border-blue-500" : "text-gray-500 dark:text-gray-400"}`}
          onClick={() => setActiveTab("Resolved")}
        >
          Resolved
        </button>
        <button
          className={`px-4 py-2 font-medium ${activeTab === "All Reports" ? "text-blue-500 border-b-2 border-blue-500" : "text-gray-500 dark:text-gray-400"}`}
          ...
          <tbody>
            {filteredReports.map((report) => (
              <tr key={report.id} className="border-t border-gray-200 dark:border-gray-700">
                <td className="px-4 py-2 font-semibold">{report.id}</td>
                <td className="px-4 py-2">{report.type}</td>
                <td className="px-4 py-2">{report.reason}</td>
                <td className="px-4 py-2">{report.reported}</td>
                ...
                <td className="px-4 py-2">{report.reports}</td>
                <td className="px-4 py-2">
                  {report.status === "Pending" ? (
